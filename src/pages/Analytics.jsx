...
export default function Analytics() {
  ...
  const pageViews = totalQuestions * 2.3;
    ...
    <div className="min-h-screen p-6 lg:p-8 bg-gray-50">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6 mb-8">
          <StatCard icon={MessageSquare} title="Total Questions" value={totalQuestions} change="+12%" trend="up" subtitle="Unique user inquiries" />
          <StatCard icon={FileText} title="Answers Provided" value={totalAnswers} change="+8%" trend="up" subtitle={`${totalQuestions > 0 ? Math.round((totalAnswers/totalQuestions)*100) : 0}% response rate`} />
          <StatCard icon={Target} title="Conversion Rate" value={`${conversionRate}%`} change="+3.2%" trend="up" subtitle="Demo bookings" />
          <StatCard icon={AlertCircle} title="Failed Answers" value={failedAnswers.length} change={failedAnswers.length > 5 ? "Action needed" : "Good"} trend={failedAnswers.length > 5 ? "down" : "up"} subtitle={`${totalQuestions > 0 ? Math.round((failedAnswers.length/totalQuestions)*100) : 0}% failure rate`} />
          <StatCard icon={Clock} title="Avg. Session Time" value={avgTimeOnEngine} change="+45s" trend="up" subtitle="Time on engine" />
        </div>

        {/* Secondary Metrics */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <StatCard icon={Users} title="Page Views" value={Math.round(pageViews)} subtitle="Total engine views" />
          <StatCard icon={Zap} title="Unique Visitors" value={uniqueVisitors} subtitle="Distinct users" />
          <StatCard icon={TrendingUp} title="Engagement Rate" value={`${totalQuestions > 0 ? Math.round((uniqueVisitors/pageViews)*100) : 0}%`} subtitle="Questions per visitor" />
        </div>
            ...
            <CardContent>
              ...
              <div className="space-y-3 max-h-[400px] overflow-y-auto">
                {failedAnswers.slice(0, 10).map((q) => (
                  <div key={q.id} className="p-4 rounded-lg border bg-white border-gray-200">
            ...
            <CardContent>
              <div className="space-y-3 max-h-[300px] overflow-y-auto">
                {topPrompts.length === 0 ? (
                  ...
                  topPrompts.map((q, i) => (
                    <div key={q.id} className="p-3 rounded-lg border bg-gray-50 border-gray-200">
          ...
          <Card className="bg-white border-gray-200">
            ...
            <CardContent className="space-y-4">
              <div>
                ...
                <div className="h-2 rounded-full overflow-hidden bg-gray-200">
                  <div className="h-full bg-pink-500" style={{ width: totalQuestions > 0 ? `${(funnelBreakdown.bottom/totalQuestions)*100}%` : '0%' }} />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="h-2 rounded-full overflow-hidden bg-gray-200">
                  <div className="h-full bg-teal-500" style={{ width: totalQuestions > 0 ? `${(totalAnswers/totalQuestions)*100}%` : '0%' }} />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="h-2 rounded-full overflow-hidden bg-gray-200">
                  <div className="h-full bg-cyan-500" style={{ width: `${conversionRate}%` }} />
                </div>
              </div>
            </CardContent>
          </Card>
