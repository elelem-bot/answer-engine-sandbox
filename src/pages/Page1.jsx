// Clone of AnswerEngineering — Page 1 (for design experimentation)
...
export default function Page1() {
  ...
  const getSeoScore = (index) => ((index * 17 + 43) % 35) + 45;
...
Be realistic with scores - good content typically scores 60-85.`,
        response_json_schema: {
          ...
          properties: {
            ...
            per_prompt_scores: {
              type: "array",
              items: { type: "object", properties: { prompt: { type: "string" }, score: { type: "number" } } }
            }
          }
        }
      });
  ...
  const sortedPrompts = useMemo(() => {
    ...
    return [...filteredPrompts].sort((a, b) => {
      ...
      if (promptSort.col === "search_signal") { av = a.search_signal_score || 0; bv = b.search_signal_score || 0; }
      else if (promptSort.col === "cluster_size") { av = ((i * 7 + 12) % 20) + 8; bv = ((j * 7 + 12) % 20) + 8; }
      else if (promptSort.col === "share") { av = ((i * 13 + 18) % 35) + 10; bv = ((j * 13 + 18) % 35) + 10; }
  ...
  const sortedPages = useMemo(() => {
    ...
    return [...rawMatchingPages].sort((a, b) => {
      ...
      if (pageSort.col === "similarity") { av = ((i * 13 + 70) % 20) + 75; bv = ((j * 13 + 70) % 20) + 75; }
      else if (pageSort.col === "citations") { av = ((i * 11 + 5) % 20) + 3; bv = ((j * 11 + 5) % 20) + 3; }
      else if (pageSort.col === "seo") { av = getSeoScore(i); bv = getSeoScore(j); }
      else if (pageSort.col === "retrieval") { av = a.relevance_score || 0; bv = b.relevance_score || 0; }
      ...
      <div className="max-w-7xl mx-auto">
        <div className="mb-8">
          <h1 className="text-2xl font-bold mb-2 text-gray-900">
            Optimize Content — Page 1
            <GuidanceDot tip="This tool helps you optimise existing pages or create new ones to improve your AI search visibility." position="right" />
          </h1>
                        ...
                        <tbody>
                          {matchingPages.map((page, i) => (
                            ...
                            >
                              ...
                              <td className="p-4 text-center"><span className="font-semibold text-purple-600">{((i * 13 + 70) % 20) + 75}%</span></td>
                              <td className="p-4 text-center"><span className="font-semibold text-blue-600">{((i * 11 + 5) % 20) + 3}</span></td>
                              <td className="p-4 text-center">
                                {(() => {
                              ...
                              <td className="p-4 text-center"><span className="text-teal-600 font-semibold">{page.relevance_score}/100</span></td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
                  ...
                  <CardContent className="p-4 space-y-4">
                    ...
                    {draftContent && !isGeneratingDraft && (
                      <>
                        <Textarea
                          value={editorContent}
                          onChange={(e) => { setEditorContent(e.target.value); setRescoreResult(null); }}
                              ...
                              <div className="flex items-start gap-4">
                                ...
                                <div className="flex-1 pt-1">
                                  <div className="w-full bg-gray-100 rounded-full h-3 mb-2">
                                    <div className="h-3 rounded-full bg-gradient-to-r from-teal-600 to-teal-500 transition-all duration-700" style={{ width: `${rescoreResult.retrieval_score}%` }} />
                                  </div>
                                  <p className="text-xs text-gray-500">Current: {rescoreResult.retrieval_score} | <span className="text-teal-600 font-semibold">Change: +{rescoreResult.retrieval_score}</span></p>
                                </div>
                              </div>
                              {rescoreResult.summary && <p className="text-xs text-gray-500 mt-3 border-t border-gray-100 pt-3">{rescoreResult.summary}</p>}
                            </div>
                            {(rescoreResult.per_prompt_scores || []).length > 0 && (
                              <div className="rounded-lg border border-gray-200 bg-white p-5">
                                <p className="text-sm font-semibold text-gray-700 mb-4">Per-Prompt Scores</p>
                                <div className="space-y-4">
                                  {rescoreResult.per_prompt_scores.map((item, i) => (
                                    <div key={i} className="border-b border-gray-100 pb-4 last:border-0 last:pb-0">
                                      ...
                                      <p className="text-xs text-gray-500">Current: {item.score} | <span className="text-teal-600 font-semibold">Change: +{item.score}</span></p>
                                    </div>
                                  ))}
                                </div>
                              </div>
                        ...
                        <Button className="w-full" variant="outline" onClick={handleRescore} disabled={isRescoring}>
                          {isRescoring ? <><Loader2 className="w-4 h-4 mr-2 animate-spin" />Rescoring...</> : <><RotateCcw className="w-4 h-4 mr-2" />Rescore Draft</>}
                        </Button>
                      </>
                    )}
                  </CardContent>
